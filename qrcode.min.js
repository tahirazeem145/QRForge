...
// Adapted for canvas & SVG rendering with colors, quiet zones, and error correction.
(function (root, factory) {
    ...
    //-----------------------------------------------------------------------
    // QRCode Generator Core (Kazuhiko Arase)
    //-----------------------------------------------------------------------
    ...
    return {
        ...
        renderCanvas: function (canvas, text, options) {
            options = options || {};
            var eclMap = { 'L': QRErrorCorrectionLevel.L, 'M': QRErrorCorrectionLevel.M, 'Q': QRErrorCorrectionLevel.Q, 'H': QRErrorCorrectionLevel.H };
            var ecLevel = eclMap[options.errorCorrectionLevel] !== undefined ? eclMap[options.errorCorrectionLevel] : QRErrorCorrectionLevel.M;
            
            var qr = new QRCodeModel(-1, ecLevel);
            qr.addData(text);
            qr.make();

            var count = qr.getModuleCount();
            var size = options.size || 256;
            var margin = options.margin !== undefined ? parseInt(options.margin, 10) : 2;
            ...
            for (var row = 0; row < count; row++) {
                for (var col = 0; col < count; col++) {
                    if (qr.isDark(row, col)) {
                        ...
                        var w = Math.ceil((col + margin + 1) * cellSize) - x;
                        var h = Math.ceil((row + margin + 1) * cellSize) - y;
        ...
        renderSVGString: function (text, options) {
            options = options || {};
            var eclMap = { 'L': QRErrorCorrectionLevel.L, 'M': QRErrorCorrectionLevel.M, 'Q': QRErrorCorrectionLevel.Q, 'H': QRErrorCorrectionLevel.H };
            var ecLevel = eclMap[options.errorCorrectionLevel] !== undefined ? eclMap[options.errorCorrectionLevel] : QRErrorCorrectionLevel.M;
            ...
            return '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" viewBox="0 0 ' + totalModules + ' ' + totalModules + '">' +
